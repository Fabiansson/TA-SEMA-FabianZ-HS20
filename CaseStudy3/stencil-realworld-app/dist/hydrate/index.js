...
const NAMESPACE = 'app';
const BUILD = /* app */ { allRenderFn: false, appendChildSlotFix: false, asyncLoading: true, attachStyles: true, cloneNodeFix: false, cmpDidLoad: true, cmpDidRender: false, cmpDidUnload: true, cmpDidUpdate: true, cmpShouldUpdate: false, cmpWillLoad: true, cmpWillRender: false, cmpWillUpdate: false, connectedCallback: true, constructableCSS: false, cssAnnotations: true, cssVarShim: false, devTools: false, disconnectedCallback: true, dynamicImportShim: false, element: false, event: false, hasRenderFn: true, hostListener: false, hostListenerTarget: false, hostListenerTargetBody: false, hostListenerTargetDocument: false, hostListenerTargetParent: false, hostListenerTargetWindow: false, hotModuleReplacement: false, hydrateClientSide: true, hydrateServerSide: true, hydratedAttribute: false, hydratedClass: true, isDebug: false, isDev: false, isTesting: false, lazyLoad: true, lifecycle: true, lifecycleDOMEvents: false, member: true, method: false, mode: false, observeAttribute: true, profile: false, prop: true, propBoolean: true, propMutable: true, propNumber: true, propString: true, reflect: true, safari10: false, scoped: false, scriptDataOpts: false, shadowDelegatesFocus: false, shadowDom: false, shadowDomShim: true, slot: true, slotChildNodesFix: false, slotRelocation: true, state: true, style: true, svg: false, taskQueue: true, updatable: true, vdomAttribute: true, vdomClass: true, vdomFunctional: true, vdomKey: true, vdomListener: true, vdomPropOrAttr: true, vdomRef: true, vdomRender: true, vdomStyle: true, vdomText: true, vdomXlink: true, watchCallback: true };
...
class AppRoot {
  ...
  render() {
    ...
    return (hAsync("div", null, hAsync("app-header", { user: user, signOut: this.signOut }), hAsync("stencil-router", null, hAsync("stencil-route-switch", { scrollTopOffset: 0 }, hAsync("stencil-route", { url: "/", component: "home-page", exact: true, componentProps: { user } }), hAsync("stencil-route", { url: "/profile/:username", component: "profile-page", exact: true, componentProps: { user } }), hAsync("stencil-route", { url: "/article/:slug", component: "article-page", exact: true, componentProps: { user } }), hAsync("stencil-route", { url: "/settings", component: isLogged ? 'settings-page' : 'not-found', exact: true, componentProps: {
        setUser: this.setUser,
        user,
      } }), hAsync("stencil-route", { url: ['/editor', '/editor/:slug'], component: isLogged ? 'editor-page' : 'not-found', exact: true, componentProps: { user } }), hAsync("stencil-route", { url: ['/login', '/register'], component: isLogged ? 'not-found' : 'auth-page', componentProps: { setUser: this.setUser }, exact: true }), hAsync("stencil-route", { component: "not-found" }))), hAsync("app-footer", null)));
  }
...
class ArticleList {
  ...
  render() {
    if (this.errors || !Array.isArray(this.articles)) {
      return [hAsync("p", null, "There was an error fetching recent articles, details below"), hAsync("error-display", { errors: this.errors })];
    }
    if (this.articles.length === 0) {
      return hAsync("p", null, "No results found");
    }
    return this.articles.map((a, i) => (hAsync("div", { class: "article-preview" }, hAsync("div", { class: "article-meta" }, hAsync("stencil-route-link", { url: `/profile/${a.author.username}` }, hAsync("img", { src: a.author.image, alt: "author image" })), hAsync("div", { class: "info" }, hAsync("stencil-route-link", { url: `/profile/${a.author.username}`, anchorClass: "author" }, a.author.username), hAsync("span", { class: "date" }, new Date(a.updatedAt).toLocaleDateString('en', {
      month: 'long',
      day: '2-digit',
      year: 'numeric',
    }))), hAsync("button", { class: `btn btn-sm pull-xs-right ${a.favorited ? 'btn-primary' : 'btn-outline-primary'}`, "data-article-slug": a.slug, "data-article-favorited": a.favorited ? '1' : '0', "data-article-index": i, disabled: !this.user, onClick: this.favoriteArticle }, hAsync("ion-icon", { name: "heart" }), " ", a.favoritesCount)), hAsync("stencil-route-link", { url: `/article/${a.slug}`, anchorClass: "preview-link" }, hAsync("h1", null, a.title), hAsync("p", null, a.description), hAsync("span", null, "Read more..."), a.tagList && (hAsync("ul", { class: "tag-list" }, a.tagList.map(t => (hAsync("li", { class: "tag-default tag-pill tag-outline" }, t)))))))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
...
class ArticleMeta {
  ...
  render() {
    ...
    return (hAsync("div", { class: "article-meta" }, hAsync("stencil-route-link", { url: `/profile/${username}` }, hAsync("img", { src: image, alt: "author image" })), hAsync("div", { class: "info" }, hAsync("stencil-route-link", { url: `/profile/${username}`, anchorClass: "author" }, username), hAsync("span", { class: "date" }, new Date(updatedAt).toLocaleDateString('en', {
      month: 'long',
      day: '2-digit',
      year: 'numeric',
    }))), isOwner
      ? [
        hAsync("stencil-route-link", { anchorClass: "btn btn-outline-secondary btn-sm", url: `/editor/${slug}` }, hAsync("ion-icon", { name: "ion-edit" }), " Edit Article"),
        hAsync("button", { class: "btn btn-outline-danger btn-sm", "aria-label": `Click to delete this article`, onClick: this.deleteArticle }, hAsync("ion-icon", { name: "ion-trash-a" }), " Delete Article"),
      ]
      : this.user && [
        hAsync("button", { class: `btn btn-sm ${following ? 'btn-outline-secondary' : 'btn-secondary'}`, "aria-label": `Click to follow ${username}`, onClick: this.followAuthor }, !following && hAsync("ion-icon", { name: "ion-plus-round" }), "\u00A0 ", !following ? 'Follow' : 'Unfollow', " ", username),
        hAsync("button", { class: `btn btn-sm ${favorited ? 'btn-primary' : 'btn-outline-primary'}`, "aria-label": `Click to favorite this article`, onClick: this.favoriteArticle }, hAsync("ion-icon", { name: "heart" }), "\u00A0 ", favorited ? 'Unfavorite' : 'Favorite', " Article ", hAsync("span", { class: "counter" }, "(", favoritesCount, ")")),
      ]));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
...
class ArticlePage {
  ...
  render() {
    if (this.isLoading) {
      return hAsync("loading-spinner", null);
    }
    if (this.errors) {
      return [hAsync("h1", null, "Something went wrong"), hAsync("error-display", { errors: this.errors })];
    }
    if (this.notFound || !this.article) {
      return hAsync("not-found", null);
    }
    ...
    return (hAsync("main", { class: "article-page" }, hAsync("div", { class: "banner" }, hAsync("div", { class: "container" }, hAsync("h1", null, a.title), hAsync("article-meta", Object.assign({}, metaProps)))), hAsync("div", { class: "container page" }, hAsync("div", { class: "row article-content" }, hAsync("div", { class: "col-md-12", innerHTML: marked(a.body) }), this.article.tagList && (hAsync("ul", { class: "tag-list" }, this.article.tagList.map(t => (hAsync("li", { class: "tag-default tag-pill tag-outline" }, t)))))), hAsync("hr", null), hAsync("div", { class: "article-actions" }, hAsync("article-meta", Object.assign({}, metaProps))), hAsync("div", { class: "row" }, hAsync("div", { class: "col-xs-12 col-md-8 offset-md-2" }, user ? (hAsync("comment-form", { slug: slug, user: this.user, addComment: this.addComment })) : (hAsync("p", null, hAsync("stencil-route-link", { url: "/login" }, "Sign in"), " or ", hAsync("stencil-route-link", { url: "/register" }, "sign up"), " to comment.")), this.comments && this.comments.map(c => hAsync("single-comment", { user: this.user, comment: c, slug: slug, removeComment: this.removeComment })))))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
...
class AsyncContent {
  ...
  render() {
    return (hAsync("div", { innerHTML: this.content }));
  }
...
class AuthPage {
  ...
  render() {
    ...
    return (hAsync("main", { class: "auth-page" }, hAsync("div", { class: "container page" }, hAsync("div", { class: "row" }, hAsync("div", { class: "col-md-6 offset-md-3 col-xs-12" }, hAsync("h1", { class: "text-xs-center" }, title), hAsync("p", { class: "text-xs-center" }, hAsync("stencil-route-link", { url: isRegister ? '/login' : '/register' }, isRegister ? 'Have an account?' : 'Need an account?')), hAsync("error-display", { errors: this.errors }), hAsync("form", { onSubmit: this.handleSubmit }, isRegister && (hAsync("fieldset", { class: "form-group", disabled: this.disabled }, hAsync("input", { class: "form-control form-control-lg", type: "text", placeholder: "Your Name", value: this.name, onInput: this.handleChange, "data-auth-id": "name", required: true }))), hAsync("fieldset", { class: "form-group", disabled: this.disabled }, hAsync("input", { class: "form-control form-control-lg", type: "email", placeholder: "Email", value: this.email, onInput: this.handleChange, "data-auth-id": "email", required: true })), hAsync("fieldset", { class: "form-group", disabled: this.disabled }, hAsync("input", { class: "form-control form-control-lg", type: "password", placeholder: "Password", value: this.password, onInput: this.handleChange, "data-auth-id": "password", required: true })), hAsync("button", { class: "btn btn-lg btn-primary pull-xs-right", type: "submit", disabled: this.disabled }, title)))))));
  }
  static get watchers() { return {
    "match": ["updateTitle"]
  }; }
...
class CommentForm {
  ...
  render() {
    if (this.errors) {
      return hAsync("error-display", { errors: this.errors });
    }
    if (!this.user) {
      return (hAsync("p", null, "Please ", hAsync("stencil-route-link", { url: "/login" }, "sign in"), " or ", hAsync("stencil-route-link", { url: "/register" }, "register"), " to comment."));
    }
    return (hAsync("form", { class: "card comment-form", onSubmit: this.handleSubmit }, hAsync("fieldset", { disabled: this.disabled }, hAsync("div", { class: "card-block" }, hAsync("textarea", { class: "form-control", placeholder: "Write a comment...", rows: 3, value: this.body, onInput: this.handleChange, required: true })), hAsync("div", { class: "card-footer" }, hAsync("img", { src: this.user.image, class: "comment-author-img", alt: "comment author image" }), hAsync("button", { class: "btn btn-sm btn-primary", type: "submit" }, "Post Comment")))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
...
class EditorPage {
  ...
  render() {
    ...
    return (hAsync("main", { class: "editor-page" }, hAsync("div", { class: "container page" }, hAsync("div", { class: "row" }, hAsync("div", { class: "col-md-10 offset-md-1 col-xs-12" }, hAsync("form", { onSubmit: this.handleSubmit }, hAsync("fieldset", { disabled: this.disabled }, inputFields.map(i => {
      const props = {
      ...
      return hAsync("fieldset", { class: "form-group" }, i.isTextArea ? hAsync("textarea", Object.assign({}, props, { rows: 8 })) : hAsync("input", Object.assign({}, props)));
    }), hAsync("button", { class: "btn btn-lg pull-xs-right btn-primary", type: "submit" }, "Publish Article"))))))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
    "$tagName$": "editor-page",
...
class ErrorDisplay {
  ...
  render() {
    ...
    return (hAsync("ul", { class: "error-messages" }, Object.keys(this.errors).map(k => this.errors[k].map(e => (hAsync("li", null, k, " ", e))))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
    "$tagName$": "error-display",
    "$members$": {
      "errors": [16]
    },
    "$listeners$": undefined,
...
class Footer {
  ...
  render() {
    return (hAsync("footer", null, hAsync("div", { class: "container" }, hAsync("stencil-route-link", { url: "/", anchorClass: "logo-font" }, "conduit"), hAsync("span", { class: "attribution" }, "An interactive learning project from ", hAsync("a", { href: "https://thinkster.io" }, "Thinkster"), ". Code & design licensed under MIT."))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
    "$tagName$": "app-footer",
...
class Header {
  ...
  render() {
    const { user } = this;
    return (hAsync("nav", { class: "navbar navbar-light" }, hAsync("div", { class: "container" }, hAsync("stencil-route-link", { class: "navbar-brand", url: "/" }, "conduit"), hAsync("ul", { class: "nav navbar-nav pull-xs-right" }, hAsync("li", { class: "nav-item" }, hAsync("stencil-route-link", { anchorClass: "nav-link", url: "/", activeClass: "active", exact: true }, "Home")), this.user
      ? [
        hAsync("li", { class: "nav-item" }, hAsync("stencil-route-link", { anchorClass: "nav-link", url: "/editor" }, hAsync("ion-icon", { name: "compose" }), "\u00A0New Post")),
        hAsync("li", { class: "nav-item" }, hAsync("stencil-route-link", { anchorClass: "nav-link", url: "/settings" }, hAsync("ion-icon", { name: "gear-a" }), "\u00A0Settings")),
        hAsync("li", { class: "nav-item" }, hAsync("stencil-route-link", { anchorClass: "nav-link", url: `/profile/${this.user.username}` }, user.image && hAsync("img", { class: "user-pic", src: user.image, alt: "user profile image" }), user.username)),
        hAsync("li", { class: "nav-item" }, hAsync("button", { class: "nav-link", onClick: this.signOut }, "Sign out")),
      ]
      : [
        hAsync("li", { class: "nav-item" }, hAsync("stencil-route-link", { anchorClass: "nav-link", url: "/login", activeClass: "active" }, "Sign in")),
        hAsync("li", { class: "nav-item" }, hAsync("stencil-route-link", { anchorClass: "nav-link", url: "/register", activeClass: "active" }, "Sign up")),
      ]))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
...
class HomePage {
  ...
  render() {
    return (hAsync("main", { class: "home-page" }, hAsync("div", { class: "banner" }, hAsync("div", { class: "container" }, hAsync("h1", { class: "logo-font" }, "conduit"), hAsync("p", null, "A place to share your knowledge."))), hAsync("div", { class: "container page" }, hAsync("div", { class: "row" }, hAsync("tabbed-feed", { class: "col-md-9", user: this.user, clearTag: this.setTag, activeTag: this.activeTag, possibleTabs: this.user ? ['global', 'feed'] : ['global'] }), hAsync("home-tags", { class: "col-md-3", setTag: this.setTag })))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
    "$tagName$": "home-page",
...
class HomeTags {
  ...
  render() {
    if (this.tags.length === 0) {
      return hAsync("div", { class: "sidebar" });
    }
    return (hAsync("div", { class: "sidebar" }, hAsync("p", null, "Popular Tags"), hAsync("div", { class: "tag-list" }, this.tags.map(t => (hAsync("button", { type: "button", class: "tag-pill tag-default", "data-tag-id": t, onClick: this.handleClick }, t))))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
    "$tagName$": "home-tags",
...
class LoadingSpinner {
  ...
  render() {
    return hAsync("div", { class: "loader" });
  }
...
class ProfilePage {
  ...
  render() {
    ...
    return (hAsync("main", { class: "profile-page" }, hAsync("div", { class: "user-info" }, hAsync("div", { class: "container" }, hAsync("div", { class: "row" }, hAsync("div", { class: "col-xs-12 col-md-10 offset-md-1" }, hAsync("img", { src: image, class: "user-img", alt: "user image" }), hAsync("h4", null, username), bio && hAsync("p", null, bio), this.user && this.user.username !== username && (hAsync("button", { class: `btn btn-sm action-btn ${following ? 'btn-secondary' : 'btn-outline-secondary'}`, onClick: this.followProfile }, !following && hAsync("ion-icon", { name: "ion-plus-round" }), "\u00A0 ", following ? 'Unfollow' : 'Follow', " ", username)))))), hAsync("div", { class: "container" }, hAsync("div", { class: "row" }, hAsync("div", { class: "col-xs-12 col-md-10 offset-md-1" }, hAsync("tabbed-feed", { class: "col-md-9", user: this.user, profile: this.profile, possibleTabs: ['authored', 'favorited'], isProfile: true }))))));
  }
  static get watchers() { return {
    "match": ["getNewUser"]
  }; }
...
var ActiveRouter = createProviderConsumer({
    ...
    root: '/',
    routeViewsUpdated: () => { }
}, (subscribe, child) => (hAsync("context-consumer", { subscribe: subscribe, renderer: child })));
...
class Route {
  ...
  render() {
    ...
    if (this.component) {
      ...
      return (hAsync(ChildComponent, Object.assign({}, childProps)));
    }
  }
  get el() { return getElement(this); }
  static get watchers() { return {
...
class RouteLink {
  ...
  render() {
    ...
    return (hAsync(this.custom, Object.assign({}, anchorAttributes), hAsync("slot", null)));
  }
  get el() { return getElement(this); }
  static get watchers() { return {
    "location": ["computeMatch"]
...
class Router {
  ...
  render() {
    ...
    return (hAsync(ActiveRouter.Provider, { state: state }, hAsync("slot", null)));
  }
  get el() { return getElement(this); }
  static get cmpMeta() { return {
    "$flags$": 4,
...
class SettingsPage {
  ...
  render() {
    return (hAsync("main", { class: "settings-page" }, hAsync("div", { class: "container page" }, hAsync("div", { class: "row" }, hAsync("div", { class: "col-md-6 offset-md-3 col-xs-12" }, hAsync("h1", { class: "text-xs-center" }, "Your Settings"), hAsync("error-display", { errors: this.errors }), hAsync("form", { onSubmit: this.handleSubmit }, hAsync("fieldset", { disabled: this.disabled }, inputFields$1.map(i => {
      const props = {
      ...
      return hAsync("fieldset", { class: "form-group" }, i.isTextArea ? hAsync("textarea", Object.assign({}, props, { rows: 8 })) : hAsync("input", Object.assign({}, props)));
    }), hAsync("button", { class: "btn btn-lg btn-primary pull-xs-right" }, "Update Settings"))))))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
...
class SingleComment {
  ...
  render() {
    ...
    return (hAsync("div", { class: "card" }, hAsync("div", { class: "card-block" }, hAsync("p", { class: "card-text" }, body)), hAsync("div", { class: "card-footer" }, hAsync("stencil-route-link", { url: authorURL, anchorClass: "comment-author" }, hAsync("img", { class: "comment-author-img", src: author.image, alt: "comment author image" })), "\u00A0", hAsync("stencil-route-link", { url: authorURL, anchorClass: "comment-author" }, author.username), hAsync("span", { class: "date-posted" }, "Dec 29th"), hAsync("span", { class: "date-posted" }, new Date(updatedAt).toLocaleDateString('en', {
      month: 'short',
      day: 'numeric',
    })), this.user && this.user.username === author.username && (hAsync("span", { class: "mod-options" }, hAsync("button", { "aria-label": "Button to delete your comment", onClick: this.deleteComment }, hAsync("ion-icon", { name: "ion-trash-a" })))))));
  }
  static get cmpMeta() { return {
    "$flags$": 0,
    "$tagName$": "single-comment",
    "$members$": {
      "user": [16],
      "comment": [16],
      "slug": [1],
...
class TabbedFeed {
  ...
  render() {
    ...
    return [
      hAsync("div", { class: wrapperClass }, hAsync("ul", { class: "nav nav-pills outline-active" }, this.possibleTabs.map(t => (hAsync("li", { class: "nav-item" }, hAsync("button", { onClick: this.toggleTab, "data-tab-id": t, class: `nav-link ${activeTab === t ? 'active disabled' : ''}`, type: "button", "aria-label": `Button to toggle your ${t} feed`, disabled: activeTab === t }, tabLabels[t])))), activeTag && (hAsync("li", { class: "nav-item" }, hAsync("span", { class: `nav-link ${activeTab === 'tag' ? 'active disabled' : ''}` }, hAsync("ion-icon", { name: "ion-pound" }), " ", activeTag))))),
      this.isLoading ? (hAsync("loading-spinner", null)) : ([
        hAsync("article-list", { listedArticles: this.articles, errors: this.errors, user: this.user }),
        count > perPage && (hAsync("ul", { class: "pagination" }, pagesArray.map((p, i) => (hAsync("li", {
        ...
        }, hAsync("button", { class: "page-link", onClick: this.goToPage, "data-page-num": i }, i + 1)))))),
      ]),
    ];
  }
  static get watchers() { return {
  ...
  static get cmpMeta() { return {
    ...
    "$members$": {
      ...
      "profile": [16],
      "clearTag": [16],
      "activeTag": [1, "active-tag"],
      "isProfile": [4, "is-profile"],
      "activeTab": [32],
      "currentPage": [32],
      "articles": [32],
      "articlesCount": [32],
